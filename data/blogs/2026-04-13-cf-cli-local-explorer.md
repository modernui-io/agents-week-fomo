...
authors: ["Matt “TK” Taylor", "Dimitri Mitropoulos", "Dan Carter", "Michelle Chen", "Kevin Flansburg", "Vlad Krasnov", "Ming Lu", "Gabriel Massadas", "Miguel Cardoso", "Anni Wang", "Vy Ton", "Matt Silverlock"]
---

# Building a CLI for all of Cloudflare

By Matt “TK” Taylor, Dimitri Mitropoulos, Dan Carter, Michelle Chen, Kevin Flansburg, Vlad Krasnov, Ming Lu, Gabriel Massadas, Miguel Cardoso, Anni Wang, Vy Ton, Matt Silverlock | 2026-04-13
...
- [![Matt “TK” Taylor](https://blog.cloudflare.com/cdn-cgi/image/format=auto,dpr=3,width=64,height=64,gravity=face,fit=crop,zoom=0.5/https://cf-assets.www.cloudflare.com/zkvhlag99gkb/4kMUByJja8M8uVuoV6RpPm/b6c1174a075f270890a0315bc7705949/Matt_TK_Taylor_.webp)

](/author/matt-tk-taylor/)[Matt “TK” Taylor](/author/matt-tk-taylor/)
- [![Dimitri Mitropoulos](https://blog.cloudflare.com/cdn-cgi/image/format=auto,dpr=3,width=64,height=64,gravity=face,fit=crop,zoom=0.5/https://cf-assets.www.cloudflare.com/zkvhlag99gkb/MBhaw6IJyDRpBgHdxbfYB/84f1ab71592c3c472040ee68afe404ea/dimitri-mitropoulos.webp)

](/author/dimitri-mitropoulos/)[Dimitri Mitropoulos](/author/dimitri-mitropoulos/)
- [![Dan Carter](https://blog.cloudflare.com/cdn-cgi/image/format=auto,dpr=3,width=64,height=64,gravity=face,fit=crop,zoom=0.5/https://cf-assets.www.cloudflare.com/zkvhlag99gkb/3tRYEmEChIKhiMW2ATLDBd/85db120f24be4cec425d1b97c0a2446b/d2c8a679-8b69-490d-b899-53d87920a3c2_800x800.png)

](/author/dan-carter/)[Dan Carter](/author/dan-carter/)

6 min read![](https://cf-assets.www.cloudflare.com/zkvhlag99gkb/6TTRJoUvbs5eWPtnu6NuL6/d31b8479cfca7f4a77517f875f0049eb/BLOG-3224_1.png)

Cloudflare has a vast API surface. We have over 100 products, and nearly 3,000 HTTP API operations.

Increasingly, agents are the primary customer of our APIs. Developers bring their coding agents to build and deploy [applications](https://workers.cloudflare.com/solutions/frontends), [agents](https://workers.cloudflare.com/solutions/ai), and [platforms](https://workers.cloudflare.com/solutions/platforms) to Cloudflare, configure their account, and query our APIs for analytics and logs.

We want to make every Cloudflare product available in all of the ways agents need. For example, we now make Cloudflare’s entire API available in a single Code Mode MCP server that uses [less than 1,000 tokens](https://blog.cloudflare.com/code-mode-mcp/). There’s a lot more surface area to cover, though: [CLI commands](https://developers.cloudflare.com/workers/wrangler/commands/). [Workers Bindings](https://blog.cloudflare.com/workers-environment-live-object-bindings/) — including APIs for local development and testing. [SDKs](https://developers.cloudflare.com/fundamentals/api/reference/sdks/) across multiple languages. Our [configuration file](https://developers.cloudflare.com/workers/wrangler/configuration/). [Terraform](https://developers.cloudflare.com/terraform/). [Developer docs](https://developers.cloudflare.com/). [API docs](https://developers.cloudflare.com/api/) and OpenAPI schemas. [Agent Skills](https://github.com/cloudflare/skills).

Today, many of our products aren’t available across every one of these interfaces. This is particularly true of our CLI — [Wrangler](https://developers.cloudflare.com/workers/wrangler/). Many Cloudflare products have no CLI commands in Wrangler. And agents love CLIs.
...
## Rethinking schemas and our code generation pipeline from first principles
    ...
    We already generate the Cloudflare [API SDKs](https://blog.cloudflare.com/lessons-from-building-an-automated-sdk-pipeline/), [Terraform provider](https://blog.cloudflare.com/automatically-generating-cloudflares-terraform-provider/), and [Code Mode MCP server](https://blog.cloudflare.com/code-mode-mcp/) based on the OpenAPI schema for Cloudflare API. But updating our CLI, Workers Bindings, wrangler.jsonc configuration, Agent Skills, dashboard and docs is still a manual process. This was already error-prone, required too much back and forth, and wouldn’t scale to support the whole Cloudflare API in the next version of our CLI.


          ![](https://cf-assets.www.cloudflare.com/zkvhlag99gkb/2wjmgUzBkjeI0RtyXKIMXm/f7fc6ce3b7323aacb3babdfb461f383f/BLOG-3224_2.png)
...
We write a lot of TypeScript at Cloudflare. It’s the lingua franca of software engineering. And we keep finding that it just works better to express APIs in TypeScript — as we do with [Cap n’ Web](https://blog.cloudflare.com/capnweb-javascript-rpc-library/), [Code Mode](https://blog.cloudflare.com/code-mode/), and the [RPC system](https://developers.cloudflare.com/workers/runtime-apis/rpc/) built into the Workers platform.
...
So we introduced a new TypeScript schema that can define the full scope of APIs, CLI commands and arguments, and context needed to generate any interface. The schema format is “just” a set of TypeScript types with conventions, linting, and guardrails to ensure consistency. But because it is our own format, it can easily be adapted to support any interface we need, today or in the future, while still *also* being able to generate an OpenAPI schema:
          ...
          ![](https://cf-assets.www.cloudflare.com/zkvhlag99gkb/4H0xSIPMmrixUWsFL86RUJ/998b93a465d26d856885b4d833ac19d4/BLOG-3224_3.png)
...
Wrangler CLI is also fairly unique — it provides commands and configuration that can work with both simulated local resources, or remote resources, like [D1 databases](https://developers.cloudflare.com/d1/), [R2 storage buckets](https://developers.cloudflare.com/r2), and [KV namespaces](https://developers.cloudflare.com/kv). This means consistent defaults matter even more. If an agent thinks it’s modifying a remote database, but is actually adding records to local database, and the developer is using remote bindings to develop locally against a remote database, their agent won’t understand why the newly-added records aren’t showing up when the agent makes a request to the local dev server. Consistent defaults, along with output that clearly signals whether commands are applied to remote or local resources, ensure agents have explicit guidance.
...
Local Explorer lets you introspect the simulated resources that your Worker uses when you are developing locally, including [KV](https://www.cloudflare.com/developer-platform/products/workers-kv/), [R2](https://www.cloudflare.com/developer-platform/products/r2/), D1, [Durable Objects](https://www.cloudflare.com/developer-platform/products/durable-objects/) and [Workflows](https://www.cloudflare.com/developer-platform/products/workflows/). The same things you can do via the Cloudflare API and Dashboard with each of these, you can also do entirely locally, powered by the same underlying API structure.

For years we’ve [made a bet on fully local development](https://blog.cloudflare.com/wrangler3/) — not just for Cloudflare Workers, but for the entire platform. When you use D1, even though D1 is a hosted, serverless database product, you can run your database and communicate with it via bindings entirely locally, without any extra setup or tooling. Via [Miniflare](https://developers.cloudflare.com/workers/testing/miniflare/), our local development platform emulator, the Workers runtime provides the exact same APIs in local dev as in production, and uses a local SQLite database to provide the same functionality. This makes it easy to write and run tests that run fast, without the need for network access, and work offline.
...
Jump into the [Cloudflare Developers Discord](https://discord.cloudflare.com/) and tell us what you’d like us to add first to the CLI, and stay tuned for many more updates soon.
...
To learn more about our mission to help build a better Internet, [start here](https://www.cloudflare.com/learning/what-is-cloudflare/). If you're looking for a new career direction, check out [our open positions](https://www.cloudflare.com/careers).  [Developers](/tag/developers/)[Developer Platform](/tag/developer-platform/)[Cloudflare Workers](/tag/workers/)[Product News](/tag/product-news/)[D1](/tag/d1/)[API](/tag/api/)[Agents Week](/tag/agents-week/)
